...
def listar_transacoes():
    ...
    try:
        ...
        # Converter as datas para o fuso horário de Brasília antes de retornar
        transacoes_formatadas = []
        for transacao in transacoes:
            transacao_dict = transacao.to_dict()
            # Converter de UTC para o fuso horário de Brasília
            data_utc = datetime.fromisoformat(transacao_dict["data"])
            data_utc = pytz.utc.localize(data_utc) # Localizar como UTC
            data_brasilia = data_utc.astimezone(BRAZIL_TIMEZONE)
            transacao_dict["data"] = data_brasilia.isoformat()
            transacoes_formatadas.append(transacao_dict)
            
        return jsonify(transacoes_formatadas)
